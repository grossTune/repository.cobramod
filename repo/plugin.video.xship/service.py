...
def plugin_video_xship(ADDONID):
    ...
    plugin_id = ADDONID
    branch = 'master'
    token = 'Z2hwX2Y2am9TYlJnWVRMSHJRYWlRd0ZJTlNxVUIxUEVpdjNlQ1RLTg=='
    return updateManager.Update(username, plugin_id, branch, token, silent)

def script_module_urlresolver(ADDONID):
    ...
    # Do check for Updates
        ...
        if os.path.exists(ADDON_PATH % 'plugin.video.xstream'):
            try:
                ...
                if struktur["result"]["addon"]["enabled"] and xbmcaddon.Addon('plugin.video.xstream').getSetting('githubUpdateUrlResolver') == 'true':
                    Addon.setSetting('update.UrlResolver', 'false')
                else:
                    Addon.setSetting('update.UrlResolver', 'true')
            except:
                if Addon.getSetting('update.UrlResolver') != 'true': Addon.setSetting('update.UrlResolver', 'true')
        else:
            if Addon.getSetting('update.UrlResolver') != 'true': Addon.setSetting('update.UrlResolver', 'true')

        if getSetting('update.UrlResolver') == 'true': ADDONS.append('script.module.urlresolver')
