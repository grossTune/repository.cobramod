...
class source:
    def __init__(self):
        ...
        self.base_link = 'https://movie4k.wiki'
        ...
        try:
            ...
            for sSearchText in titles:
                try:
                    ...
                    pattern = 'movie-item.*?href="([^"]+).*?<h3>([^<]+).*?white">([^<]+)'
    ...
    def chk_link(self, sUrl, sName, quality, hostDict):
        ...
        valid, hoster = source_utils.is_host_valid(sUrl, hostDict)
        if not valid: return
        hmf = urlresolver.HostedMediaFile(url=sUrl, include_disabled=True, include_universal=False)
        if hmf.valid_url():
            url = hmf.resolve()
