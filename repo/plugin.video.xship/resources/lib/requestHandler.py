...
import io, gzip, time, re
import socket, os, sys, hashlib, json

try:
    import xbmcgui
    from resources.lib.control import dataPath, addonName
...
class cRequestHandler:
    def __init__(self, sUrl, caching=True, ignoreErrors=True, compression=True, jspost=False):
        self._sUrl = sUrl
        self._sRealUrl = ''
        ...
        self.jspost = jspost
        self.cacheTime = 600  # int(cConfig().getSetting('cacheTime', 600))
        self.requestTimeout = 10  # int(cConfig().getSetting('requestTimeout', 10))
    ...
    def request(self):
        ...
        try:
            ...
            if e.code == 503:
                oResponse = None
                if not oResponse:
                    if self.printErrors: print('Failed Cloudflare aktiv Url: ' + self._sUrl)
            if e.code == 403:
                data = e.fp.read()
                if 'DDOS-GUARD' in str(data):
                    ...
                    if not oResponse:
                        if not self.ignoreErrors:
                            print('Failed DDOS-GUARD Url: ' + self._sUrl)
                        return ''
                else:
                    return ''
            elif not self.ignoreErrors:
                xbmcgui.Dialog().ok(ADDON_NAME, 'Fehler beim Abrufen der Url: {0} {1}'.format(self._sUrl, str(e)))
                if self.printErrors: print('HTTPError ' + str(e) + ' Url: ' + self._sUrl)
                return ''
            else:
                oResponse = e
        except URLError as e:
            if not self.ignoreErrors:
                xbmcgui.Dialog().ok(ADDON_NAME, str(e.reason))
            if self.printErrors: print('URLError ' + str(e.reason) + ' Url: ' + self._sUrl)
            return ''
        except HTTPException as e:
            if not self.ignoreErrors:
                xbmcgui.Dialog().ok(ADDON_NAME, str(e))
            if self.printErrors: print('HTTPException ' + str(e) + ' Url: ' + self._sUrl)
            return ''
        ...
        if 'lazingfast' in sContent:
            ...
            if bf:
                ...
                if self.printErrors: print('Failed BF Url: ' + self._sUrl)
        try:
            cookieJar.save(ignore_discard=self.__bIgnoreDiscard, ignore_expires=self.__bIgnoreExpired)
        except Exception as e:
            if self.printErrors: print('Failed save cookie: %s' % e)
    ...
    def readCache(self, url):
        ...
        if 0 < fileAge < self.cacheTime:
            try:
                ...
                if self.printErrors: print('Could not read Cache')
            if content:
                if self.printErrors: print('read html for %s from cache' % url)
                return content
        return ''
    ...
    def writeCache(self, url, content):
        try:
            ...
            if self.printErrors: print('Could not write Cache')
...
class RedirectFilter(HTTPRedirectHandler):
    def redirect_request(self, req, fp, code, msg, hdrs, newurl):
        if 'notice.cuii' in newurl: #'https://notice.cuii.info/'
            return None
