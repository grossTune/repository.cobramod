...
class MyProgressDialog():
    def __init__(self, process):
        ...
        self.dp.create("[B][COLORgoldenrod]STUBE.BOX.matrix.LiTE UPDATE[/COLOR][/B]", "[CR]".join((process, '', 'Bitte warten...')))
...
def get_update(version):
    ...
    bundleURL = "http://stubebox.stube.org/wizard/stubeboxML/stubeboxML.zip"
    yesnowindow = xbmcgui.Dialog().yesno('[B][COLORgoldenrod]STUBE.BOX.matrix.LiTE UPDATE[/COLOR][/B]', 'Ein Update ist verfügbar![CR]Möchtest du das Update installieren?', yeslabel='JA', nolabel='SPÄTER')
    ...
    if yesnowindow == 1:
        ...
        if not exists(bundleURL):
            xbmcgui.Dialog().ok('STUBE.BOX.matrix.LiTE UPDATE [B][COLORorange]OFFLINE[/COLOR][/B]','Aktuell leider nicht verfügbar.Versuche es später nochmal...' )
            sys.exit() 
        ...
        try:
            ...
            if extract(io.BytesIO(data), addon_folder, MyProgressDialog("Installation...")):
                message = "Update von STUBE.BOX.matrix.LiTE erfolgreich abgeschlossen."
                succ = True
            else:
        ...
        if zipped:
            try:
                ...
                pass
        dialog = xbmcgui.Dialog()
        dialog.ok(addon_name, "%s. STUBE.BOX.matrix.LiTE wird beendet, um den Vorgang abzuschliessen....." % message)
        os._exit(0)
    else:
        dialog.ok('[B][COLORgoldenrod]STUBE.BOX.matrix.LiTE UPDATE[/COLOR][/B]','Update nicht durchgeführt! Beim nächsten Start wird erneut gefragt...')