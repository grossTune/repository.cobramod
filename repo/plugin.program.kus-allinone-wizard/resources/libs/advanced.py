...
def remove_current():
    ...
    ok = dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]Willst du die aktuelle AdvancedSettings wirklich entfernen?[/COLOR]".format(CONFIG.COLOR2),
...
class AdvancedMenu:
    ...
    def show_menu(self, url=None):
        ...
        if os.path.exists(CONFIG.ADVANCED):
            directory.add_file('Aktuelle AdvancedSettings anzeigen',
                               {'mode': 'advanced_settings', 'action': 'view_current'}, icon=CONFIG.ICONADVANCED,
                               themeit=CONFIG.THEME1)
            directory.add_file('Aktuelle AdvancedSettings entfernen',
                               {'mode': 'advanced_settings', 'action': 'remove_current'}, icon=CONFIG.ICONADVANCED,
                               themeit=CONFIG.THEME1)
                    ...
                    for preset in presets:
                        ...
                        if section:
                            directory.add_dir(name, {'mode': 'advanced_settings', 'url': preseturl},
                                              description=description, icon=icon, fanart=fanart, themeit=CONFIG.THEME1)
                        else:
        ...
        if response:
            if os.path.exists(CONFIG.ADVANCED):
                choice = self.dialog.yesno(CONFIG.ADDONTITLE,
                                           "[COLOR {0}]Willst du die aktuellen AdvancedSettings mit [COLOR {1}]{2}[/COLOR] überschreiben?[/COLOR]".format(
                                               CONFIG.COLOR2, CONFIG.COLOR1, name),
                                           yeslabel="[COLORlime]Ja[/COLOR]",
                                           nolabel="[COLORred]Nein[/COLOR]")
            else:
                choice = self.dialog.yesno(CONFIG.ADDONTITLE,
                                           "[COLOR {0}]Willst du die AdvancedSettings für[CR][COLOR {1}]{2}[/COLOR] herunterladen und installieren?[/COLOR]".format(
                                               CONFIG.COLOR2, CONFIG.COLOR1, name),
                                           yeslabel="[COLORlime]Ja[/COLOR]",
                                           nolabel="[COLORred]Nein[/COLOR]")
            ...
            if choice == 1:
                ...
                tools.kill_kodi(msg='[COLOR {0}]Die neuen AdvancedSettings wurden erfolgreich installiert.[CR]Um die Änderungen zu übernehmen muss Kodi neu gestartet werden![/COLOR]'.format(
                                   CONFIG.COLOR2))
            else:
