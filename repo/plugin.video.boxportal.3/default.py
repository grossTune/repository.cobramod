...
addon_id = 'plugin.video.boxportal.3'
...
livetv = \
    xbmc.translatePath(os.path.join('special://home/addons/plugin.video.boxportal.3/resources/pic/icon.png'
                       ))
filme = \
    xbmc.translatePath(os.path.join('special://home/addons/plugin.video.boxportal.3/resources/pic/icon.png'
                       ))
...
with urllib.request.urlopen("https://raw.githubusercontent.com/cobramod/repository.cobramod/main/repo/tv/3/live.txt") as url:
    portal = url.read().decode('utf-8')

with urllib.request.urlopen("https://raw.githubusercontent.com/cobramod/repository.cobramod/main/repo/tv/3/user.txt") as url2:
    username = url2.read().decode('utf-8')

with urllib.request.urlopen("https://raw.githubusercontent.com/cobramod/repository.cobramod/main/repo/tv/3/pass.txt") as url3:
...
Guide = \
    xbmc.translatePath(os.path.join('special://home/addons/plugin.video.boxportal.3/resources/catchup'
                       , 'guide.xml'))
GuideLoc = \
    xbmc.translatePath(os.path.join('special://home/addons/plugin.video.boxportal.3/resources/catchup'
                       , 'g'))
...
def stream_video(url):
    ...
    progress.create('BOX.PORTAL', u'Der Stream wird gestartet...')
    progress.update(15)
...
def searchdialog():
    search = control.inputDialog(heading='BOX.PORTAL Suche:')
    if search == '':
        return
    else:
        return search
...
def addonsettings(url, description):
    if url == 'CC':
        ...
        if dialog == 0:
            ...
            xbmcgui.Dialog().ok('BOX.PORTAL', 'Set Advanced Settings')
        elif dialog == 1:
        ...
        xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.boxportal.3/resources/modules/speedtest.py")'
                            )
    elif url == 'META':
...
def correctPVR():
    ...
    addon = xbmcaddon.Addon('plugin.video.boxportal.3tPVRtv')
...
def tvguidesetup():
    dialog = xbmcgui.Dialog().yesno('BOX.PORTAL',
    ...
    if dialog:
        ...
        if dialog == 0:
            ivueint()
            xbmcgui.Dialog().ok('BOX.PORTAL', 'iVue Integration Complete'
                                )
        elif dialog == 1:
            pvrsetup()
            xbmcgui.Dialog().ok('BOX.PORTAL', 'PVR Integration Complete')
        elif dialog == 2:
            pvrsetup()
            ivueint()
            xbmcgui.Dialog().ok('BOX.PORTAL',
                                'PVR & iVue Integration Complete')
